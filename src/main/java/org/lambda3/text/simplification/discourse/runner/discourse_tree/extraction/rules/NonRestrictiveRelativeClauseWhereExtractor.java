 ...
 *
 */
public class NonRestrictiveRelativeClauseWhereExtractor extends ExtractionRule {
	
	@Override
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        TregexPattern p = TregexPattern.compile("ROOT <<: (S << (/.*/=head < (NP|PP $+ (/,/=comma $+ (SBAR=sbar <, (WHADVP $+ S=s & <<: WRB) & ?$+ /,/=comma2)))))");
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            RelationType relation = RelationType.SPATIAL;
