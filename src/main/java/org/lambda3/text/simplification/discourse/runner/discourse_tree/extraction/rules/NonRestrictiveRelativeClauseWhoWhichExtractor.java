 ...
 *
 */
public class NonRestrictiveRelativeClauseWhoWhichExtractor extends ExtractionRule {
	
	@Override
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        TregexPattern p = TregexPattern.compile("ROOT <<: (S << (NP=head <, NP=np & < (/,/=comma $+ (SBAR=sbar <, (WHNP $+ S=s & <<: WP|WDT) & ?$+ /,/=comma2))))");
