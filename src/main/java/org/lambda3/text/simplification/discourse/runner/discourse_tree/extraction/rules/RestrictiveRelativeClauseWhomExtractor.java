 ...
 *
 */
public class RestrictiveRelativeClauseWhomExtractor extends ExtractionRule {
    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        TregexPattern p = TregexPattern.compile("ROOT <<: (S << (NP=head <, NP=np & < (SBAR=sbar <, (WHNP $+ (S=s <, NP=np2 & <- (VP=vp ?<+(VP) PP=prep)) & <<: (WP <: whom)))))");
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            RelationType relation = classifer.classifySubordinating(cuePhraseWords).orElse(RelationType.IDENTIFYING_DEFINITION);
