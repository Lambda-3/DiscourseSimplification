...
public class LeadNPExtractor extends ExtractionRule {
    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        TregexPattern p = TregexPattern.compile("ROOT <<: (S <, (NP=np $+ (/,/ $+ NP & $++ VP=vp)))");
        TregexMatcher matcher = p.matcher(leaf.getParseTree());

        while (matcher.findAt(leaf.getParseTree())) {

        	// rephrase
            List<Word> leftConstituentWords = rephraseEnablement(matcher.getNode("np"), matcher.getNode("vp"));
            ...
            rightConstituentWords.addAll(ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), matcher.getNode("np"), false));
