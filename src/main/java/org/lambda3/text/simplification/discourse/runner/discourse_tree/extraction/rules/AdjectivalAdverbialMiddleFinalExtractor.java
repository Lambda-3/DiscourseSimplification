 ...
 *
 */
public class AdjectivalAdverbialMiddleFinalExtractor extends ExtractionRule {
	
	@Override
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
		
        TregexPattern p = TregexPattern.compile("ROOT <<: (S < VP=vp & << (/,/=comma1 $+ (ADJP|ADVP=ad ?$+ /,/=comma2)))");
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            rightConstituentWords.addAll(ParseTreeExtractionUtils.getPrecedingWords(leaf.getParseTree(), matcher.getNode("comma1"), false));
            ...
            // relation
            RelationType relation = RelationType.UNKNOWN_SUBORDINATION; //TODO
