 ...
 *
 */
public class RestrictiveRelativeClauseWhoseExtractor extends ExtractionRule {
    ...
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        ...
        TregexPattern p = TregexPattern.compile("ROOT <<: (S << (NP=head < (NP=np $+ (SBAR=sbar <, (WHNP $+ S=s & <, (/WP\\$/ $+ /.*/=nn))))))");
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            RelationType relation = RelationType.IDENTIFYING_DEFINITION;
