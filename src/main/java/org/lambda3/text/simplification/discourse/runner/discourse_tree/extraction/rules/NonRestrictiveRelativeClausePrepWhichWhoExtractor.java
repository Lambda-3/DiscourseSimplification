 ...
 *
 */
public class NonRestrictiveRelativeClausePrepWhichWhoExtractor extends ExtractionRule {
	
	@Override
    public Optional<Extraction> extract(Leaf leaf) throws ParseTreeException {
        TregexPattern p = TregexPattern.compile("ROOT <<: (S << (NP=head <, NP=np & < (/,/=comma $+ (SBAR=sbar <, (WHPP $+ S=s & <, IN=prep & <- WHNP) & ?$+ /,/=comma2))))");
        ...
        while (matcher.findAt(leaf.getParseTree())) {
            ...
            rightConstituentWords.addAll(ParseTreeExtractionUtils.getWordsInBetween(leaf.getParseTree(), matcher.getNode("np"), matcher.getNode("comma"), true, false));
            ...
            RelationType relation = classifer.classifySubordinating(cuePhraseWords).orElse(RelationType.DESCRIBING_DEFINITION);
